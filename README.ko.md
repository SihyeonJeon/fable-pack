...
[`examples/trace-bugfix/`](examples/trace-bugfix/) — 이 repo의 실제 버그 수정 trace 정제본: goal 해석, 컨텍스트 선정 사유, 결정 이벤트(기각 대안 포함), 관찰, 검증 보고, 재구성 [타임라인](examples/trace-bugfix/timeline.txt)
...
- 모델 간 프로세스 이전 — 게이트 통과 trace 코퍼스가 다른 모델 세션의 계획 품질을 실제로 올리는지: shadow 비교쌍 + 채점 증거 필요 (도구는 있음, 벤치마크는 아직)
- shadow delta의 의미 기반 비교
- 커뮤니티 룰팩 갤러리
